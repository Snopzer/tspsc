<?php
	...
	$subjectQuery = $conn->query("SELECT * FROM r_subject")or die(mysqli_error());
	$subjectCount = mysqli_num_rows($subjectQuery);
	$pages = $subjectCount / ADMIN_PAGE_LIMIT;
	$pages = ceil($pages);
	...
	$userList = $conn->query("SELECT * FROM r_subject order by id_subject desc limit $page1,".ADMIN_PAGE_LIMIT);
?>  
                        ...
                        <div class="horz-grid">
                            <div class="bs-example">
                                <table class="table">
                                    <tbody>
                                        <tr>
                                            <td><h3 id="h3.-bootstrap-heading"> SUBJECTS - [<?php echo $subjectCount; ?>]</h></td>
                                            <td class="type-info text-right">
                                                <a href="subject.php?action=add"><span class="btn btn-success"><i class="fa fa-plus-square white" aria-hidden="true"></i> <span class="desktop"> <?php echo ADD_BUTTON;?></span></span></a> 
                                    ...
                                    <?php
										if (mysqli_num_rows($userList) > 0) {
											...
											<tr class="table-row <?php echo ($user["status"]==1)?'warning':'danger'; ?>">
                                                <td class="table-img"><input type="checkbox" name="selectcheck" value="<?= $user["id_subject"] ?>"/></td>
                                                <td class="march"><h6><?php echo $user["name"] ?></h6></td>
                                                <td class="march"><h6><?php echo ($user["status"] == 1) ? 'Enable' : 'Disable'; ?></h6></td>
                                                <td><a href="subject.php?id=<?php echo $user["id_subject"] ?>&action=edit&page=<?php echo "$page" ?>"><span class="label label-primary"><i class="fa fa-pencil white" aria-hidden="true"></i></span><a/>
												<a href="subject-controller.php?chkdelids=<?php echo $user["id_subject"] ?>&action=delete&page=<?php echo "$page" ?>""><span class="label label-info"><i class="fa fa-remove white" aria-hidden="true"></i></span></a>
                                                </td>
											</tr>
                                            <?php
											}
                            ...
                            <?php
								if ($subjectCount > ADMIN_PAGE_LIMIT) {
								?>
                                <div class="horz-grid text-center">
                                    <ul class="pagination pagination-lg">
                                        ...
                                        <?php for ($b = 1; $b <= $pages; $b++) { ?>
                                            <?php if ($b == $page) { ?>
                                                <li class="active"><a href="subject.php?page=<?php echo $b; ?>"><?php echo $b . " "; ?></a></li>    
												<?php } else { ?>
                                                <li><a href="subject.php?page=<?php echo $b; ?>"><?php echo $b . " "; ?></a></li>
                                                <?php
												}
											}
										?>
            ...
            <div class="content-main">
                <div class="banner">
                    <h2>
                        <a href="home.php">Home</a>
                        <i class="fa fa-angle-right"></i>
                        <span><a href="subject.php">subject</a></span>
                        <i class="fa fa-angle-right"></i>
                        <span><?php echo ($_GET['action'] == 'edit') ? 'Edit subject' : 'Add subject'; ?></span>
					</h2>
				</div>
                <div class="grid-system">
                    <div class="horz-grid">
                        <div class="grid-hor">
                            <h4 id="grid-example-basic"><?php echo ($_GET['action'] == 'edit') ? 'Edit subject' : 'Add subject'; ?> Details:</h4>
                        ...
                        <?php
							if ($_GET['action'] == "edit") {
								...
								$page = $_GET['page'];
								$query = $conn->query("select * from r_subject where id_subject=$id")or die(mysql_error());
								$result =$query->fetch_assoc();
							?>
                            <form class="form-horizontal" action="subject-controller.php" method="post">
	...
	<script language="JavaScript">
        function fnDetails()
        {
			...
			} else if (flag == 1)
            {
                ...
                {
                    window.location.href = "subject.php?action=edit&page=<? echo "$page"?>&uid=" + arrval[0];
				}
			}
		}
	</script>
        ...
        function fnDelete()
        {
            ...
            if (flag == 0) {
                ...
                if (ConfirmStatus == true) {
                    ...
                    document.frmMain.action = "subject-controller.php";
